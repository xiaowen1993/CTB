...
public class BranchPharmacyCache {
	 ...
	 * @return
	 */
	public BranchPharmacy getBranchPharmacy(String pharmacyId) {
	    BranchPharmacy branchPharmacy = null;
	            
        Set<Object> fields = redisClient.hKeys(getBranchPharmacyCacheKey());
        Collection<Object> result = Collections2.filter(fields, new Predicate<Object>() {
            @Override
            public boolean apply(Object obj) {
                
                return ((String) obj).contains(pharmacyId);
            }
        });
        ...
        if (!CollectionUtils.isEmpty(result)) {
            ...
            branchPharmacy = JSONObject.parseObject(JSONObject.toJSONString(object), BranchPharmacy.class);
        }
