...
/**
 * 
 * @ClassName: com.ctb.framework.commons.generator.OrderNoGenerator
 * @Description: TODO(订单号生成类)
 * @author cwq
 * @date 2019年3月22日 下午4:26:21
 */
public class OrderNoGenerator {
     ...
     * 
     * @platformType 平台类型(2位) 微信服务窗（01），支付宝服务窗（02），银联中嵌入app（03）
     * @payType 支付类型(2位) 微信支付（01），支付宝支付（02），微信支付（健康易app）（03）
     * @tradeType 交易类型(1位) 支付（1），退费（2），部分退费（3）
     * @bizType 业务编号(1位) 挂号（1），门诊（2），住院（3），AI处方（4）
     * @serverNo 服务器编号(2位,不足2位的前补0,根据IP/MAC生成)
     * @openId 生成3位哈希值
     */
    public static String genOrderNo(String platformType, String payType, int tradeType, int bizType, String serverNo,
            String openId) {
    ...
    public static String getBizType(String orderNo) {
    	...
    	return orderNo.substring(19, 20);
    }
    ...
    public static String getHashVal(String orderNo) {
    	...
    	return orderNo.substring(22, 25);
